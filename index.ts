/**
 * Created by mariakaranasou on 5/27/17.
 */
...
let scrollVis = function () {
    ...
    // color value.
    // @v4 using new scale name
    let coughColorScale = d3.scaleLinear()
        .domain([0, 1.0])
        .range(['#008080', 'red']);
    ...
    let chart = function (selection) {
        selection.each(function (rawData) {
            ...
            let wordData = getWords(rawData);
            // filter to just include filler words
            let fillerWords = getFillerWords(wordData);

            // get the counts of filler words for the
            // bar chart display
            let fillerCounts = groupByWord(fillerWords);
            // set the bar scale's domain
            let countMax = d3.max(fillerCounts, function (d) { return d.value;});
            xBarScale.domain([0, countMax]);

            // get aggregated histogram data

            let histData = getHistogram(fillerWords);
            // set histogram's domain
            let histMax = d3.max(histData, function (d) { return d.length; });
     ...
     */
    let setupVis = function (wordData, fillerCounts, histData) {
        // axis
        g.append('g')
            .attr('class', 'x axis')
        ...
        let squares = g.selectAll('.square').data(wordData, function (d) { return d.word; });
        let squaresE = squares.enter()
            .append('rect')
            .classed('square', true);
        ...
        // @v4 Using .merge here to ensure
        // new and old data have same attrs applied
        let bars = g.selectAll('.bar').data(fillerCounts);
        let barsE = bars.enter()
            .append('rect')
            .attr('class', 'bar');
        ...
        let barText = g.selectAll('.bar-text').data(fillerCounts);
        barText.enter()
            .append('text')
            .attr('class', 'bar-text')
        ...
        // @v4 Using .merge here to ensure
        // new and old data have same attrs applied
        let hist = g.selectAll('.hist').data(histData);
        let histE = hist.enter().append('rect')
            .attr('class', 'hist');
    ...
    function getHistogram(data) {
        ...
        let thirtyMins = data.filter(function (d) { return d.min < 30; });
     ...
     */
    chart.prototype.activate = function (index) {
        activeIndex = index;
     ...
     * @param progress
     */
    chart.prototype.update = function (index, progress) {
...
function display(data) {
    ...
    let scroll = scroller().prototype.container(d3.select('#graphic'));
    ...
    scroll.on('active', function (index) {
        ...
        plot.prototype.activate(index);
    });

    scroll.on('progress', function (index, progress) {
        plot.prototype.update(index, progress);
    });
}
